...
if(isset($_POST['search'])) {
	fetchData($_POST, 'security');
	die();
}elseif(isset($_GET['update'])){
	updateData($_GET, 'security');
	die();
}
...
?>
	...
	<script type="text/javascript">
					...
					success: function(response) {
						if(response != 0) {
							...
							for(i=0; i< aa.length; i++){
								...
								html += "<td class='srpending_case'>"+((aa[i]['pending_case'] == null)? 0: aa[i]['pending_case'])+"</td>";
		...
		function showUpdate(me) {
			...
			var pending_case = $(a+'>.srpending_case').html();
		
			html = "<div class='col-sm-2 col-sm-offset-4'>";
			...
			html += "<label style='display:block;' for='pending_case'>Pending Case</label>";
			html += "<input name='pending_case' type='checkbox' "+((pending_case == '0') ? 'unchecked': 'checked')+" /></div>";
